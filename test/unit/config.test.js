...
test("uses fast, bounded fallback defaults", async () => {
    const configUrl = new URL("../../src/media-core/config.js", import.meta.url).href;
    const script = `import { config } from ${JSON.stringify(configUrl)}; process.stdout.write(JSON.stringify({ yt: config.ytdlpRetries, ytTimeout: config.ytdlpSocketTimeoutSeconds, gallery: config.galleryDlRetries, galleryTimeout: config.galleryDlHttpTimeoutSeconds }));`;
    const env = { ...process.env };
    ...
    assert.deepEqual(JSON.parse(stdout), { yt: 1, ytTimeout: 15, gallery: 1, galleryTimeout: 15 });
});
